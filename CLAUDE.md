...
extract_wave_exposure.R   - Derive per-MPA wave exposure (HSMAX) from Bell 2023 EDI NetCDF -> data/per_mpa_wave_exposure.csv (standalone)
...
Derived per-MPA covariate files in `data/` (tracked; small):
  ...
  per_mpa_wave_exposure.csv - mean HSMAX wave exposure per MPA (Bell 2023 sbc.162 NetCDF; all 34 MPAs incl. islands; used by 16)
                              cols: MPA, Lat, Lon, nearest_km, n_stations, wave_hs (m), island, coverage
...
**Environmental-covariate gap (done):** `16_environmental_moderators.R` tests the environmental predictors Kumagai mapped in their PCA but never modeled (MHW & cold-spell intensity, latitude, MPA size, and **wave exposure**) as meta-regression moderators of our per-MPA effect sizes (Knapp-Hartung). Clean null: nothing survives FDR; giant-kelp resilience is not modulated by any gradient (incl. wave exposure, all p≥0.35); omitting these covariates does not appear to bias the headline conclusions. WAVE DATA (island-inclusive): per-MPA mean HSMAX (max significant wave height) from **Bell (2023), EDI knb-lter-sbc.162** ("California kelp canopy and environmental variable dynamics"; the per-pixel NetCDF behind Kumagai's hsmax and Wanner et al. 2024's Channel-Islands analysis; CDIP MOP v1.1 waves). Extracted by `code/R/extract_wave_exposure.R` → `data/per_mpa_wave_exposure.csv` (ALL 34 MPAs, incl. islands; raw NetCDF + provenance in `~/sbc-kelp-env/`). Range 0.65 m (Catalina lee, sheltered) → 4.9 m (San Miguel Is., exposed), correctly recovering the island exposed/sheltered gradient. The earlier mainland-only tabular product (knb-lter-sbc.144) is superseded. nitrate/depth still not reconstructed. See the "Environmental-moderator supplement" note above.

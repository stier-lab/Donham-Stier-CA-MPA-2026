...
| Figure S / internal (candidate) | Heatwave × MPA cascade: per-taxon lnRR (emmeans ± 95% CI) across before/during/after the 2014-16 MHW, RR-scaled axis, Southern California. Companion to Kumagai et al. 2024. | `fig_heatwave_cascade.pdf` | 14_heatwave_analysis.R |
...
| **Figure 5 (ACCEPTED, main text)** | Giant-kelp resistance & recovery: inside-vs-outside biomass trajectory through the 2014-16 MHW (single column). **The one resilience result going into the main text.** | `fig_kelp_resilience.pdf` | 22_resistance_recovery.R |
...
Environmental-moderator supplement (16_environmental_moderators.R) tests whether the MPA effect varies across the environmental gradients Kumagai et al. (2024) MAPPED in their PCA (Fig 3) but never put in a model — closing the one covariate gap surfaced by script 15. Per-MPA covariates obtainable without fabrication: MHW cumulative intensity during 2014-16 (thermal stress; their 1-km raster at MPA centroids), cold-spell cumulative intensity (thermal regime/upwelling; their 1-km grid, nearest cell), latitude, log MPA size. Per taxon, random-effects meta-regression (`metafor::rma`, **Knapp-Hartung** small-sample t-test — essential at k=9-15; density for animals, biomass for kelp), BH-FDR across all tests. (Wave exposure, nitrate, and human gravity were later added island-inclusive from Bell 2023 EDI sbc.162 + Kumagai gravity grid -- see the "Environmental-covariate gap (DONE)" note below for the full 35-test result.) Tables: `table_s_mpa_env_covariates.csv`, `table_s_env_moderators.csv`; section + legend in `docs/methods_comparison_supplement.md`.
KEY: **clean null — no moderator survives FDR.** Strongest = red urchin effect declines with MHW intensity (-1.63 lnRR/SD, p=0.007, FDR=0.21; suggestive only). **Giant kelp effect is NOT modulated by any gradient** (all p≥0.17) → resilience is not an artifact of thermal/biogeographic reserve placement. Practical upshot: omitting Kumagai's environmental covariates does not appear to bias the headline conclusions. (Without Knapp-Hartung the same k=9-15 regressions give implausible p down to 1e-18 — overconfident IV weighting.)
...
24_resilience_pipeline_check.R - Concordance gate for Figure 5 values, pair counts, and SI-supporting resilience outputs
...
`run_all.R` then runs `24_resilience_pipeline_check.R`; this is the paper/SI drift
gate for the main-text resilience paragraph, Figure 5, and the SI-supporting
resilience outputs.
...
Derived per-MPA covariate files in `data/` (tracked; small):
  ...
  per_mpa_kelp_env.csv      - Bell 2023 sbc.162 NetCDF env covariates + human gravity per MPA, all 34 MPAs incl. islands (used by 16/18)
                              cols: MPA, Lat, Lon, nearest_km, n_stations, wave_hs (m), nitrate (uM), temperature (C), npp, depth_m, gravity, island, coverage
  heatwave_exposure_SBC_annual.csv - SBC-region annual MHW days/intensity + period (heatwaveR/OISST; used by 14/19)
...
**Ecological memory (done):** `23_ecological_memory.R` asks whether the SAME reserves respond in both heatwaves (script 19 showed average-level repeatability; this tests reserve-level consistency). KEY: yes — a reserve's MHW1 response predicts its MHW2 response (kelp Spearman ρ=0.71 p=0.028; purple urchin Pearson r=0.87 p=0.001), no priming (MHW2≈MHW1 magnitude). Resilience is a consistent reserve property. Reconciles with script 18: reserve effectiveness is repeatable/consistent yet NOT predictable from covariates — reserves differ reliably but we can't explain why. Table `table_ecological_memory.csv`; fig `fig_ecological_memory`.

**Temporal stability (done):** `21_resilience_stability.R` tests whether protection reduces year-to-year variability (temporal CV of annual abundance, paired inside vs outside, Wilcoxon). KEY: no clear stabilization — kelp trends more stable inside (CV 0.65 vs 0.85, 64% of MPAs lower inside, but p=0.24 ns) and purple urchin is MORE variable inside (p=0.014). Resilience manifests in the MEAN kelp state, not reduced variance. Table `table_resilience_stability.csv`; fig `fig_resilience_stability`.

**SSWD / sunflower-star test (done; CORRECTED):** `20_compound_disturbance.R` tests whether the 2013-14 sea-star wasting disease (SSWD) drove the MPA cascade response (different stressor type vs Kumagai's heat; cf. Eisaguirre's SSWD focus). **A first draft was wrong** — it lumped Pycnopodia with Pisaster (which dominated counts but is NOT an urchin predator) and ignored that the sunflower star (Pycnopodia, the actual urchin predator, cold-water) was PATCHY in SoCal. Corrected: Pycnopodia was only ~7% of large-star density and present at just **8/30 reserves** (cold/N/island); ~2/3 had ~0. Because pre-SSWD Pycnopodia VARIES across reserves while the MHW hit all, we test whether the cascade response scales with sunflower-star loss. KEY (corrected): it does NOT — kelp after-before is +1.50 where sunflower stars occurred vs **+1.57 where they never did** (period×group interaction p=0.78); urchins also indistinguishable (p≥0.70). So in SoCal the cascade resilience is **heat + fishing protection, NOT sunflower-star loss/compensation**; the keystone-removal mechanism (Eisaguirre's cold NCI) does not generalize where the sunflower star was absent. This REMOVES a confound and strengthens the heatwave interpretation; SoCal should not be framed as compound SSWD for the cascade. Tables `table_compound_{pycnopodia_distribution,pycnopodia_crash,keystone_moderation,resilience,crossstudy}.csv`; fig `fig_compound_disturbance`; doc `docs/compound_disturbance.md`.

**Heatwave replication — two events (done):** `19_heatwave_replication.R` tests whether MPA resilience repeats across MHW1 (2014-16) and a second event MHW2 (2018-20, "Blob 2.0"; our series runs to 2023, catalog shows 122/26/83 heatwave-days) that script 14 buried in "after". Period re-coded before/MHW1/interim(2017)/MHW2/recent; same glmmTMB AR1+Source-RE as script 14. KEY: **giant kelp resilience REPEATS** — strongly elevated inside MPAs in BOTH events (MHW1 +1.65 p=5e-4; MHW2 +2.21 p=4e-5), the only taxon significant vs baseline in both, and it tracks heatwave intensity beyond recovery (continuous +0.32/SD p=0.016 controlling for time-since-establishment). Lobster recovery-driven (sig only MHW2); sheephead flat null; urchin suppression deepens in MHW2 (red -0.65 p=0.03; vs adjacent quiet 2017 purple -0.61 p=0.04, red -0.56 p=0.03) but before-baseline contrasts marginal under AR1 (matches script 14). First demonstration the resilience is not a one-off; also de-contaminates script 14's "after" baseline. Tables `table_heatwave_replication.csv`, `table_heatwave_replication_continuous.csv`; fig `fig_heatwave_replication`; doc `docs/heatwave_replication.md`.

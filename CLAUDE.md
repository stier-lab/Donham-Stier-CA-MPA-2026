...
**Environmental-covariate gap (done):** `16_environmental_moderators.R` tests the environmental predictors Kumagai mapped in their PCA but never modeled (MHW & cold-spell intensity, latitude, MPA size, and **wave exposure** for mainland MPAs) as meta-regression moderators of our per-MPA effect sizes (Knapp-Hartung). Clean null: nothing survives FDR; giant-kelp resilience is not modulated by any gradient (incl. wave exposure, p≥0.26); omitting these covariates does not appear to bias the headline conclusions. WAVE DATA: obtained from EDI knb-lter-sbc.144 (Bell SBC LTER swell model, same product as Kumagai's hsmax) → `data/per_mpa_wave_exposure.csv` (raw + provenance in `~/sbc-kelp-env/`). Covers MAINLAND MPAs only (14 valid ≤0.5km; the 20 Channel-Islands MPAs are 32-105km from any coastline segment → NA; island-inclusive waves need CDIP MOP or the per-pixel CAkelpCanopyEnv NetCDF). nitrate/depth still not reconstructed. See the "Environmental-moderator supplement" note above.

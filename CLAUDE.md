...
| Figure S (candidate, new) | Compound disturbance: sea-star (Pycnopodia+Pisaster) crash inside/outside + per-taxon MPA cascade response before/compound/after | `fig_compound_disturbance.pdf` | 20_compound_disturbance.R |
...
20_compound_disturbance.R - Resilience to the compound SSWD + MHW disturbance; sea-star crash + cascade reorganization (SI; standalone)
...
**Compound disturbance — SSWD + MHW (done):** `20_compound_disturbance.R` evaluates resilience to the compound 2013-14 sea-star wasting (SSWD) + 2014-16 heatwave (different stressor TYPE = biotic disease/keystone-predator loss layered on heat; complements Kumagai's MHW-only and Eisaguirre's SSWD-only). Honest framing: SSWD & MHW are temporally inseparable in SoCal → compound test, not SSWD-specific. Brings the SEA-STAR loss (absent from our 5-taxon harmonized data) from raw PISCO swath. KEY: (A) large sea stars (Pycnopodia+Pisaster) crashed ~99% at 2013-14, **equal inside (98.9%) and outside (98.8%)** — protection didn't save the star, so any cascade buffering comes from the protected fished predators. (B) the cascade REORGANIZED: before, MPAs already had more predators (lobster p=1e-5, sheephead p=2e-7) but NOT fewer urchins/more kelp (gaps ~0); after, urchin inside-outside gap widened (purple -0.88 p=0.008, red -0.75 p=0.002) and **kelp flipped from lower-inside to far higher inside (+1.78, p=7e-5)** — compensation signature (sea star controlled urchins everywhere until it died, then protected predators took over). (C) lnRR resilience holds (kelp compound +1.39 p=0.0014, after +1.56 p=2e-4), consistent with scripts 14/19. Caveats: SSWD/MHW confounded (compound, not attributable to one); predator-specific attribution fragile (per Eisaguirre repro/diversity test) but the structural reorganization is robust. Tables `table_compound_{seastar_crash,cascade_reorg,resilience,crossstudy}.csv`; fig `fig_compound_disturbance`; doc `docs/compound_disturbance.md`.

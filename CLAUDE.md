...
Derived per-MPA covariate files in `data/` (tracked; small):
  ...
  per_mpa_kelp_env.csv      - Bell 2023 sbc.162 NetCDF env covariates per MPA, all 34 MPAs incl. islands (used by 16/18)
                              cols: MPA, Lat, Lon, nearest_km, n_stations, wave_hs (m), nitrate (uM), temperature (C), npp, depth_m, island, coverage
...
**Environmental-covariate gap (done):** `16_environmental_moderators.R` tests the environmental predictors Kumagai mapped in their PCA but never modeled (MHW & cold-spell intensity, latitude, MPA size, **wave exposure**, and **nitrate/nutrients**) as meta-regression moderators of our per-MPA effect sizes (Knapp-Hartung). Clean null: 30 tests, 3 nominal p<0.05, **none survive FDR**; giant-kelp resilience is not modulated by any gradient (incl. wave exposure and nutrients); omitting these covariates does not appear to bias the headline conclusions. ENV DATA (island-inclusive): per-MPA HSMAX (wave), nitrate, temperature, npp, depth from **Bell (2023), EDI knb-lter-sbc.162** ("California kelp canopy and environmental variable dynamics"; the per-pixel NetCDF behind Kumagai's hsmax and Wanner et al. 2024's Channel-Islands analysis; CDIP MOP v1.1 waves, SST-derived nitrate). Extracted by `code/R/extract_kelp_env_covariates.R` → `data/per_mpa_kelp_env.csv` (ALL 34 MPAs incl. islands; raw NetCDF + provenance in `~/sbc-kelp-env/`). Gradients sensible: wave 0.65 m (Catalina lee) → 4.9 m (San Miguel Is.); nitrate 0.39 µM (warm San Diego) → 3.0 µM (cold upwelling, Pt Conception/NW islands). Nitrate is collinear with temperature/MHW (SST-derived). Of the Kumagai PCA covariates, only human-gravity remains unobtained (mainland-only sbc.144 superseded). See the "Environmental-moderator supplement" note above.

...
effect sizes in our pipeline), with Benjamini–Hochberg FDR across all 20 tests.
Three covariates from their PCA — per-site wave exposure (`hsmax`), nitrate, and
depth — could not be reconstructed per MPA (their per-site values came from a
kelp-canopy NetCDF absent from the public mirror; only a single *regional* SBC wave
series exists locally, with no among-MPA variation), and are flagged as a
recommended future addition rather than estimated from inadequate data.
...
happen to sit thermally, biogeographically, or by size. The Knapp–Hartung

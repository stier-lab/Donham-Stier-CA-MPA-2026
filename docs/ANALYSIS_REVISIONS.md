...
- **The tricky part:** Red urchin *biomass* is higher inside MPAs even as density trends negative. Maybe that's larger individual urchins from less fishing pressure. Maybe habitat quality. Maybe it's a modeling artifact from the shared variance. It doesn't fit the simple cascade prediction and we need to talk through how to frame this in the Discussion.
...
6. **M. franciscanus biomass-density split:** Red urchin *biomass* is up +84% (p=0.026) inside MPAs but density is down -33% (p=0.365, non-sig). Could be reduced fishing on large reds (bigger individuals = more biomass at lower density), habitat differences, or a modeling artifact. Only significant under the joint model. We need to discuss this carefully.

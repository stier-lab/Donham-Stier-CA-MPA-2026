...
A companion **environmental-moderator analysis** (`code/R/16_environmental_moderators.R`) tests the predictors Kumagai et al. mapped in their PCA but never modeled — marine-heatwave and cold-spell intensity, latitude, MPA size, and **wave exposure** — as meta-regression moderators of our per-MPA effect sizes. Wave exposure (max significant wave height, HSMAX) is derived for **all 34 MPAs including the Channel Islands** from Bell's (2023) island-inclusive kelp-canopy-environment product (EDI `knb-lter-sbc.162`, the per-pixel NetCDF behind Kumagai's `hsmax` and Wanner et al. 2024's island analysis; `code/R/extract_wave_exposure.R` → `data/per_mpa_wave_exposure.csv`); it correctly recovers the exposed-vs-sheltered island gradient (San Miguel 4.9 m → Catalina lee 0.65 m). The result is a clean null: no moderator survives FDR correction, and giant-kelp resilience is not modulated by any environmental gradient (including wave exposure), indicating that omitting these covariates does not bias the headline conclusions. (Remaining unreconstructed PCA covariates: nitrate, depth.)

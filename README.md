...
A companion **environmental-moderator analysis** (`code/R/16_environmental_moderators.R`) tests the predictors Kumagai et al. mapped in their PCA but never modeled — marine-heatwave and cold-spell intensity, latitude, MPA size, and **wave exposure** (from the Bell SBC LTER swell-propagation product, EDI knb-lter-sbc.144 — the same family as Kumagai's `hsmax`; `data/per_mpa_wave_exposure.csv`, mainland MPAs only, as its coastline segments do not cover the Channel Islands) — as meta-regression moderators of our per-MPA effect sizes. The result is a clean null: no moderator survives FDR correction, and giant-kelp resilience is not modulated by any environmental gradient (including wave exposure), indicating that omitting these covariates does not bias the headline conclusions. (Remaining unreconstructed covariates: nitrate, depth, and island wave exposure.)

...
A companion **environmental-moderator analysis** (`code/R/16_environmental_moderators.R`) tests the predictors Kumagai et al. mapped in their PCA but never modeled — marine-heatwave and cold-spell intensity, latitude, and MPA size — as meta-regression moderators of our per-MPA effect sizes (wave exposure, nitrate, and depth are not available per-MPA without fabrication and are flagged as future additions). The result is a clean null: no moderator survives FDR correction, and giant-kelp resilience is not modulated by any environmental gradient, indicating that omitting these covariates does not bias the headline conclusions.
...
We also test resilience to a **different stressor type** — the compound **sea-star wasting disease + marine heatwave** that struck Southern California together in 2013–2016 (`code/R/20_compound_disturbance.R`, doc `docs/compound_disturbance.md`). Documenting the sea-star loss from the raw PISCO swath shows large sea stars crashed ~99% *equally inside and outside* reserves — so protection didn't save the keystone predator — and the cascade then *reorganized*: before the disturbance MPAs already held more fished predators but not fewer urchins or more kelp; after it, urchins fell and kelp flipped from lower to far higher inside reserves, the compensation signature. This is the realistic compound case (SSWD and MHW are temporally inseparable in SoCal) complementing Kumagai's heat-only and Eisaguirre's disease-only framings.

...
#   2. Handles two types of analyses:
#      - pBACIPS: Full before-after comparison with model selection (includes
#        step, linear, asymptotic, and sigmoid model types)
...
#   - pBACIPS includes step models (simple BA contrast) as well as progressive
#     change models (linear, asymptotic, sigmoid)
#   - CI analysis: Used when no before-period data exists
#
# INPUTS:
#   - All.RR.sub.trans: Combined response ratio data
#   - Site: Site metadata with MPA start years
...
nls_difference_se <- function(model, newdata_0, newdata_1, n_obs) {
  result <- tryCatch({
    ...
    if (!is.null(interval) && nrow(interval) == 2) {
      ...
      warning("nls_difference_se: Both delta method and independence fallback failed. SE set to NA.")
...
run_ci_analysis <- function(dat, taxa_name, source_name, resp_name, time_var = "time",
  ...
  for (i in seq_along(mpas)) {
    ...
    result <- tryCatch({
      ...
      # Build result rows
      # CI analysis produces two candidate rows (linear trend and simple mean).
      # Primary flag selection: if the linear slope is significant (p <= 0.05), the
      # linear model is Primary="Y" (trend is real, use it); otherwise the mean model
      # is Primary="Y" (no trend detected, simple mean is more appropriate).
      # Only Primary="Y" rows enter SumStats.Final (via the type filter).
      # Note: calculate_effect_size_from_contrast returns SE with proper covariance handling
      # SD approximation: SE * sqrt(df + 1) assumes simple regression (p = 1 predictor).
      # For models with additional parameters, this slightly overestimates SD.
      # Here df is the residual df from the linear model (n - 2 for simple regression),
      # so df + 1 ≈ n - 1 which gives the sample SD scaling.
...
add_step_effect_size <- function(dat, taxa_name, mpa_name, source_name, resp_name,
  ...
  tryCatch({
    ...
    # SD approximation: SE * sqrt(df + 1) assumes simple regression (p = 1 predictor).
    # For the step model (lm with single factor BA), df = n - 2, so df + 1 ≈ n - 1
    # which gives the sample SD scaling. For models with additional parameters,
    # this slightly overestimates SD.
    pSD <- pSE * sqrt(df_es + 1)

    # Step model is a legitimate pBACIPS result (simple before-after contrast);
    # Type = "pBACIPS" ensures it passes the SumStats.Final filter.
    # BA = "Y" indicates before-after data is available (vs "N" for CI-only analyses).
    data.frame(
...
# Use time >= 0 to include implementation year, consistent with density analysis (line 947)
...
# NOTE: Campus Point P. interruptus biomass uses time_var = "time" (raw years-since-MPA)
# rather than "time.model" because this is a CI-only site with no before-period data.
# add_time_columns() was not applied since there is no Before/After structure to encode.
...
KFM.purps.den <- subset(All.RR.sub.trans, source == "KFM" &
                           y == "Strongylocentrotus purpuratus" & resp == "Den")
...
KFM.purps.bio <- subset(All.RR.sub.trans, source == "KFM" &
...
KFM.reds.den <- subset(All.RR.sub.trans, source == "KFM" &
                          y == "Mesocentrotus franciscanus" & resp == "Den")
...
KFM.reds.bio <- subset(All.RR.sub.trans, source == "KFM" &
                          y == "Mesocentrotus franciscanus" & resp == "Bio")
  ...
  if (!is.null(sigmoid.Model)) {
    ...
    if (!is.null(interval) && nrow(interval) == 2) {
      ...
      SumStats <- rbind(SumStats, data.frame(
        Taxa = "M. franciscanus", MPA = "Harris Point SMR", Mean = mean_es, SE = pSE, SD = pSD, CI = pCI,
        Model = model_label, Source = "KFM", Resp = "Bio", BA = "Y", Primary = "Y",
        Type = "pBACIPS", LinearBefore = "N", N = n_obs_original,
        DW_stat = NA, DW_pval = NA,
        stringsAsFactors = FALSE
      ))
    }
  }
...
# NOTE: Extreme kelp effect sizes
# Some MPA-taxa combinations (particularly M. pyrifera at Scorpion SMR)
# produce very large lnRR values (e.g., lnRR ~ 9, corresponding to RR ~ 9000).
# These arise from near-zero kelp density at reference sites, which makes the
# response ratio mathematically extreme but ecologically interpretable: kelp
# was essentially absent outside the MPA. The meta-analytic framework handles
# these through inverse-variance weighting (large SEs -> low weight), so they
# do not unduly influence pooled estimates. Additionally:
# 1. Per-taxa Cook's distance outlier detection (09_meta_analysis.R, threshold 4/k)
#    flags these extreme values within the M. pyrifera subgroup.
# 2. The leave-one-out sensitivity analysis (Table S8) tests whether kelp results
#    are robust to removal of any single MPA, including Scorpion.
# 3. The three-method outlier sensitivity comparison (Table S9) confirms that
#    pooled estimates are stable across no-outlier-removal, per-taxa Cook's D,
#    and joint Cook's D approaches.
# See also: funnel plots (Fig S9) and outlier sensitivity analysis (Table S9).
...
if (nrow(KFM.macro.stipe) >= 5) {
  ...
  if (!is.null(sigmoid.Model)) {
    ...
    if (!is.null(interval) && nrow(interval) == 2) {
      ...
      SumStats <- rbind(SumStats, data.frame(
        Taxa = "M. pyrifera", MPA = "Scorpion SMR", Mean = mean_es, SE = pSE, SD = pSD, CI = pCI,
        Model = model_label, Source = "KFM", Resp = "Bio", BA = "Y", Primary = "Y",
        Type = "pBACIPS", LinearBefore = "N", N = n_obs_original,
        DW_stat = NA, DW_pval = NA,
        stringsAsFactors = FALSE
...
if (nrow(KFM.lob.gull) >= 5) {
  time.model.of.impact <- max(which(KFM.lob.gull$time.model == 0))
  time.true <- KFM.lob.gull$time.true
  sigmoid.Model <- tryCatch(
    mySIGfun_standalone(
  ...
  if (!is.null(sigmoid.Model)) {
    ...
    time.model.of.impact_original <- max(which(KFM.lob.gull$time.model == 0))
    ...
    if (!is.null(interval) && nrow(interval) == 2) {
      ...
      SumStats <- rbind(SumStats, data.frame(
        Taxa = "P. interruptus", MPA = "Gull Island SMR", Mean = mean_es, SE = pSE, SD = pSD, CI = pCI,
        Model = model_label, Source = "KFM", Resp = "Den", BA = "Y", Primary = "Y",
        Type = "pBACIPS", LinearBefore = "N", N = n_obs_original,
        DW_stat = NA, DW_pval = NA,
        stringsAsFactors = FALSE
      ))
...
SumStats.sub$Taxa <- factor(SumStats.sub$Taxa,
  levels = c("P. interruptus", "S. pulcher", "S. purpuratus", "M. franciscanus", "M. pyrifera"),
  ordered = TRUE
)
...
SumStats.sub$Type <- factor(SumStats.sub$Type,
  levels = c("pBACIPS", "CI"), ordered = TRUE
)
...
# Remove problematic MPAs using centralized EXCLUDED_MPAS constant (from 01_utils.R).
# This ensures the filter and the filter audit (Step 3 below) use the same exclusion list.
SumStats.Final <- subset(SumStats.Final, !(MPA %in% EXCLUDED_MPAS))
...
####################################################################################################
# METHODOLOGICAL NOTE: NLS fallback model implications
# When the preferred NLS models (asymptotic, sigmoid) fail to converge,
# a piecewise linear fallback is used. This fallback:
...
#    in the Model column). See outputs/model_fallback_audit.csv for details.
# The effect on pooled meta-analytic estimates is tested via outlier
# sensitivity analysis (Table S9).
#
...
####################################################################################################
## Export SumStats.Final and model selection summary to CSV #########################################
####################################################################################################
# CRITICAL: SumStats.Final is the core data table underlying all figures and meta-analyses.
# It MUST be exported so that results are reproducible and inspectable without re-running
# the full pipeline.

# --- 1. Export SumStats.Final (complete effect size table for publication) ---
sumstats_final_file <- here::here("data", "sumstats_final.csv")
write_csv(SumStats.Final, sumstats_final_file)
cat("\nSumStats.Final exported to:", sumstats_final_file, "\n")
cat("  Rows:", nrow(SumStats.Final), "\n")
cat("  Columns:", paste(names(SumStats.Final), collapse = ", "), "\n")

# --- 2. Export model selection summary (which model won for each MPA-taxa-resp) ---
# This documents the pBACIPS model selection outcome for every effect size
model_selection_summary <- SumStats.Final[, c("Taxa", "MPA", "Source", "Resp",
                                                "Model", "AnalysisType", "Mean", "SE",
                                                "CI", "N", "DW_stat", "DW_pval")]
model_selection_file <- here::here("tables", "table_model_selection.csv")
write_csv(model_selection_summary, model_selection_file)
cat("Model selection summary exported to:", model_selection_file, "\n")

# --- 3. Export sample size summary per MPA-taxa combination ---
# NOTE: Small sample sizes in per-taxa meta-analysis
# Several taxa have k <= 5 effect sizes per response type, which limits:
# 1. Power to detect true effects
# 2. Reliability of heterogeneity estimates (I-squared, tau-squared)
# 3. Validity of normality assumptions for random effects
# The pooled (all-taxa) meta-analysis with k > 50 provides the primary
# inference; per-taxa models (fit in 09_meta_analysis.R) are exploratory/
# descriptive. The sample_size_summary table below documents the exact k
# for each taxa-response combination so readers can assess precision.
sample_size_summary <- SumStats.Final %>%
  dplyr::group_by(Taxa, Resp) %>%
  dplyr::summarise(
    n_MPAs = dplyr::n_distinct(MPA),
    n_effects = dplyr::n(),
    sources = paste(sort(unique(as.character(Source))), collapse = ", "),
    models_used = paste(sort(unique(as.character(Model))), collapse = ", "),
    mean_N_per_effect = round(mean(as.numeric(N), na.rm = TRUE), 1),
    min_N = min(as.numeric(N), na.rm = TRUE),
    max_N = max(as.numeric(N), na.rm = TRUE),
    .groups = "drop"
  )
sample_size_file <- here::here("tables", "table_sample_sizes_per_taxa.csv")
write_csv(sample_size_summary, sample_size_file)
cat("Sample size summary exported to:", sample_size_file, "\n")
